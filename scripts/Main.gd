...
func submit_town() -> void:
	if input.text.strip_edges().is_empty():
		input.placeholder_text = "The carnival needs a town..."
		return
...
func submit_name() -> void:
	if input.text.strip_edges().is_empty():
		input.placeholder_text = "Enter your name first..."
		return
...
func start_gate() -> void:
	...
	clear_choices()
	add_choice("Ask the clown about the gate", gate_clown)
	add_choice("Try to force the gate open", gate_force)
	add_choice("Ignore it and enter the midway", thief_event)
	update_hud()
...
func gate_clown() -> void:
	...
	clear_choices()
	add_choice("Enter the midway", thief_event)

func gate_force() -> void:
	resolve = max(0, resolve - 5)
	record("locked_gate", "tried_to_escape", {})
...
func thief_event() -> void:
	...
	clear_choices()
	add_choice("CHASE HIM", chase_thief)
	add_choice("LET HIM GO", let_thief_go)
	add_choice("CALL FOR HELP", call_for_help)
	update_hud()
...
func chase_thief() -> void:
	...
	cash += 20
	hidden.violence += 1
	record("pickpocket", "chased", {"health_cost": 8})
	story.text = "You catch him between two tents. After a struggle, you recover the money, but leave bleeding from the elbow.\n\n[b]Cash recovered. Health -8.[/b]"
	finish_slice()

func let_thief_go() -> void:
	hidden.mercy += 1
	record("pickpocket", "let_go", {"money_lost": 20})
	story.text = "You watch him disappear. Somewhere beyond the lights, a calliope begins playing a tune you almost recognize."
	finish_slice()
...
func call_for_help() -> void:
	...
	clear_choices()
	add_choice("STOP THE WORKER", stop_worker)
	add_choice("Take your money and leave", take_money)
	add_choice("Watch", watch_worker)
	add_choice("Help the worker", help_worker)
...
func stop_worker() -> void:
	...
	hidden.mercy += 2
	hidden.sacrifice += 1
	record("worker_beating", "intervened", {"health_cost": 10})
	story.text = "You step between them. The worker shoves you hard, but finally backs away. The kid stares at you before running into the darkness.\n\n[b]Cash recovered. Health -10.[/b]"
	finish_slice()

func take_money() -> void:
	cash += 20
	hidden.gre​​ed = hidden.get("greed", 0) + 1
	record("worker_beating", "took_money", {})
	story.text = "You take the crumpled bills from the pavement and walk away. The sounds behind you continue longer than you expected."
	finish_slice()

func watch_worker() -> void:
	resolve = max(0, resolve - 10)
	hidden.cruelty += 1
	record("worker_beating", "watched", {})
...
func preview_map() -> void:
	...
	clear_choices()
	add_choice("PLAY AGAIN", reset_game)

func reset_game() -> void:
	health = 100; resolve = 100; cash = 60; food = 3; tickets = 0
	history.clear()
	for key in hidden.keys(): hidden[key] = 0
	town = ""; player_name = ""; input.text = ""
	show_town_prompt()
...
func update_hud(show_stats := true) -> void:
	hud.visible = show_stats
	hud.text = "HEALTH %d     RESOLVE %d     CASH $%d     FOOD %d     TICKETS %d" % [health, resolve, cash, food, tickets]
